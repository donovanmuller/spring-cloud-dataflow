...
public class ApplicationGroupRegistryService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ApplicationGroupRegistryService.class);
    ...
    public ApplicationGroupDescriptor extractAndParseDescriptor(MavenResource mavenResource) throws Exception {
        ...
        YamlConfigurationFactory<ApplicationGroupDescriptor> yamlConfigurationFactory = new YamlConfigurationFactory<ApplicationGroupDescriptor>(ApplicationGroupDescriptor.class);
    ...
    public void registerAndCreateApps(String name, ApplicationGroupDescriptor descriptor, boolean force) throws Exception {
        for (ApplicationGroupDescriptor.Application app : descriptor.getApps()) {
            logger.info("Registering application '{}:{}' in application group '{}'", app.getType(),
                    app.getName(), name);
            appRegistryController.register(app.getType(), app.getName(), app.getUri(), false);
        }

        for (ApplicationGroupDescriptor.Standalone standalone : descriptor.getStandalone()) {
            standaloneDefinitionController.save(standalone.getName(), standalone.getDsl(), force, false);
        }
    ...
    class MavenResourceJarExtractor {
         ...
         * @throws IOException
         *             if the file does not exist in the {@link Resource}
         */
        public Resource extractFile(Resource resource, String file)
                throws IOException {
            ...
            if (unpackedEntry != null) {
                ...
                throw new IOException(String.format("File '%s' does not exist in resource: '%s'", file, resource));
            }
        }
    }
